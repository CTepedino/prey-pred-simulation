...
def plot_extinction_time_vs_variable(x_variable_values, extinction_times, species_label, x_variable_label, variables_label):
    ...
    plt.ylabel(f"Tiempo de extinción para {species_label}", fontsize=20)
    plt.tight_layout()
...
if __name__ == '__main__':
    ...
    for N_pred in N_predators:
        ...
        prey_velocities = np.vstack(prey_mean_velocities_arr)
        prey_std = np.vstack(prey_velocities_std_arr)
        pred_velocities = np.vstack(predator_mean_velocities_arr)
        pred_std = np.vstack(predator_velocities_std_arr)
